...
extension Reactive where Base: NSObject {
     ...
     */
    func observe<Element>(_ type: Element.Type,
                                 _ keyPath: String,
...
extension Reactive where Base: NSObject {
     ...
     **Since it needs to intercept object deallocation process it needs to perform swizzling of `dealloc` method on observed object.**
     ...
     - returns: Observable sequence of objects on `keyPath`.
     */
    func observeWeakly<Element>(_ type: Element.Type, _ keyPath: String, options: KeyValueObservingOptions = [.new, .initial]) -> Observable<Element?> {
...
extension Reactive where Base: AnyObject {
    ...
    */
    ...
    private func registerMessageInterceptor<T: MessageInterceptorSubject>(_ selector: Selector) throws -> T {
        let rxSelector = RX_PREDICTORselector(selector)
        let selectorReference = RX_PREDICTORreference_from_selector(rxSelector)
        ...
        var error: NSError?
        let targetImplementation = RX_PREDICTORensure_observing(self.base, selector, &error)
...
// MARK: Message interceptors
    ...
    private final class DeallocatingProxy
        ...
        @objc var targetImplementation: IMP = RX_PREDICTORdefault_target_implementation()

        var isActive: Bool {
            return self.targetImplementation != RX_PREDICTORdefault_target_implementation()
        }
