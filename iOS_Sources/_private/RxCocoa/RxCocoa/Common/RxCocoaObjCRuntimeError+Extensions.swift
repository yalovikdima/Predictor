...
import PredictorSDK_Private
    ...
    enum RxCocoaInterceptionMechanism {
    ...
    extension Error {
        func rxCocoaErrorForTarget(_ target: AnyObject) -> RxCocoaObjCRuntimeError {
            ...
            if error.domain == RXObjCPredictorRuntimeErrorDomain {
                let errorCode = RXObjCPredictorRuntimeError(rawValue: error.code) ?? .unknown
                
                switch errorCode {
                case .unknown:
                    return .unknown(target: target)
                case .objectMessagesAlreadyBeingIntercepted:
                    let isKVO = (error.userInfo[RXObjCPredictorRuntimeErrorIsKVOKey] as? NSNumber)?.boolValue ?? false
