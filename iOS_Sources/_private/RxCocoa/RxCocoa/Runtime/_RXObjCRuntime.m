...
@class RXPredictorObjCRuntime;

BOOL RXPredictorAbortOnThreadingHazard = NO;
...
typedef BOOL (^RXInterceptWithOptimizedObserver)(RXPredictorObjCRuntime * __nonnull self, Class __nonnull class, SEL __nonnull selector, NSErrorParam error);
...
#define RX_PREDICTORPREFIX @"_RX_PREDICTORnamespace_"

#define RX_PREDICTORARG_id(value)           ((value) ?: [NSNull null])
#define RX_PREDICTORARG_char(value)         [NSNumber numberWithChar:value]
#define RX_PREDICTORARG_short(value)        [NSNumber numberWithShort:value]
#define RX_PREDICTORARG_int(value)          [NSNumber numberWithInt:value]
#define RX_PREDICTORARG_long(value)         [NSNumber numberWithLong:value]
#define RX_PREDICTORARG_BOOL(value)         [NSNumber numberWithBool:value]
#define RX_PREDICTORARG_SEL(value)          [NSNumber valueWithPointer:value]
#define RX_PREDICTORARG_rx_uchar(value)     [NSNumber numberWithUnsignedInt:value]
#define RX_PREDICTORARG_rx_ushort(value)    [NSNumber numberWithUnsignedInt:value]
#define RX_PREDICTORARG_rx_uint(value)      [NSNumber numberWithUnsignedInt:value]
#define RX_PREDICTORARG_rx_ulong(value)     [NSNumber numberWithUnsignedLong:value]
#define RX_PREDICTORARG_rx_block(value)     ((id)(value) ?: [NSNull null])
#define RX_PREDICTORARG_float(value)        [NSNumber numberWithFloat:value]
#define RX_PREDICTORARG_double(value)       [NSNumber numberWithDouble:value]
...
NSString * __nonnull const RXObjCPredictorRuntimeErrorDomain   = @"RXObjCPredictorRuntimeErrorDomain";
NSString * __nonnull const RXObjCPredictorRuntimeErrorIsKVOKey = @"RXObjCPredictorRuntimeErrorIsKVOKey";

BOOL RX_PREDICTORreturn_type_is_supported(const char *type) {
    if (type == nil) {
        return NO;
    }
...
static BOOL RX_PREDICTORmethod_has_supported_return_type(Method method) {
    ...
    return RX_PREDICTORreturn_type_is_supported(methodSignature.methodReturnType);
}

SEL __nonnull RX_PREDICTORselector(SEL __nonnull selector) {
    NSString *selectorString = NSStringFromSelector(selector);
    return NSSelectorFromString([RX_PREDICTORPREFIX stringByAppendingString:selectorString]);
}
...
id __nonnull RX_PREDICTORextract_argument_at_index(NSInvocation * __nonnull invocation, NSUInteger index) {
...
NSArray *RX_PREDICTORextract_arguments(NSInvocation *invocation) {
    ...
    for (NSUInteger index = HIDDEN_ARGUMENT_COUNT; index < numberOfArguments; ++index) {
        [arguments addObject:RX_PREDICTORextract_argument_at_index(invocation, index) ?: [NSNull null]];
    }
...
static BOOL RX_PREDICTORforward_invocation(id __nonnull __unsafe_unretained self, NSInvocation *invocation) {
    SEL originalSelector = RX_PREDICTORselector(invocation.selector);

    id<RXMessageSentObserver> messageSentObserver = objc_getAssociatedObject(self, originalSelector);

    if (messageSentObserver != nil) {
        NSArray *arguments = RX_PREDICTORextract_arguments(invocation);
...
static BOOL RX_PREDICTORresponds_to_selector(id __nonnull __unsafe_unretained self, SEL selector) {
...
static NSMethodSignatureRef RX_PREDICTORmethod_signature(id __nonnull __unsafe_unretained self, SEL selector) {
    Class class = object_getClass(self);
...
static NSString * __nonnull RX_PREDICTORmethod_encoding(Method __nonnull method) {
...
@interface RXPredictorObjCRuntime: NSObject
...
+(RXPredictorObjCRuntime*)instance;

-(void)performLocked:(void (^)(RXPredictorObjCRuntime* __nonnull))action;
 ...
 */
IMP __nullable RX_PREDICTORensure_observing(id __nonnull target, SEL __nonnull selector, NSErrorParam error) {
    ...
    @synchronized(target) {
        ...
        @synchronized([target class]) {
            [[RXPredictorObjCRuntime instance] performLocked:^(RXPredictorObjCRuntime * __nonnull self) {
...
#define FORWARD_BODY(invocation)                        if (RX_PREDICTORforward_invocation(self, NAME_CAT(_, 0, invocation))) { return; }

#define RESPONDS_TO_SELECTOR_BODY(selector)             if (RX_PREDICTORresponds_to_selector(self, NAME_CAT(_, 0, selector))) return YES;

#define CLASS_BODY(...)                                 return actAsClass;

#define METHOD_SIGNATURE_FOR_SELECTOR_BODY(selector)                                            \
    NSMethodSignatureRef methodSignature = RX_PREDICTORmethod_signature(self, NAME_CAT(_, 0, selector)); \
...
#define BUILD_ARG_WRAPPER(type)                   RX_PREDICTORARG_ ## type                                                     //RX_PREDICTORARG_ ## type

#define CAT(_1, _2, head, tail)                   RX_PREDICTORCAT2(head, tail)
#define SEPARATE_BY_COMMA(_1, _2, head, tail)     head, tail
#define SEPARATE_BY_SPACE(_1, _2, head, tail)     head tail
#define SEPARATE_BY_UNDERSCORE(head, tail)        RX_PREDICTORCAT2(RX_PREDICTORCAT2(head, _), tail)

#define UNDERSCORE_TYPE_CAT(_1, index, type)      RX_PREDICTORCAT2(_, type)                                                    // generates -> _type
...
#define EXAMPLE_PARAMETER(_1, index, type)        RX_PREDICTORCAT2(_, type):(type)SEPARATE_BY_UNDERSCORE(type, index)          // generates -> _type:(type)type_0
#define SELECTOR_PART(_1, index, type)            RX_PREDICTORCAT2(_, type:)                                                   // generates -> _type:

#define COMMA_DELIMITED_ARGUMENTS(...)            RX_PREDICTORFOREACH(_, SEPARATE_BY_COMMA, NOT_NULL_ARGUMENT_CAT, ## __VA_ARGS__)
#define ARGUMENTS(...)                            RX_PREDICTORFOREACH_COMMA(_, NAME_CAT, ## __VA_ARGS__)
#define DECLARE_ARGUMENTS(...)                    RX_PREDICTORFOREACH_COMMA(_, TYPE_AND_NAME_CAT, ## __VA_ARGS__)

// optimized observe methods

#define GENERATE_SELECTOR_IDENTIFIER(...)         RX_PREDICTORCAT2(exampleSelector, RX_PREDICTORFOREACH(_, CAT, UNDERSCORE_TYPE_CAT, ## __VA_ARGS__))

#define GENERATE_METHOD_IDENTIFIER(...)           RX_PREDICTORCAT2(swizzle, RX_PREDICTORFOREACH(_, CAT, UNDERSCORE_TYPE_CAT, ## __VA_ARGS__))
...
#define BUILD_EXAMPLE_METHOD(return_value, ...) \
    +(return_value)RX_PREDICTORCAT2(RX_PREDICTORCAT2(example_, return_value), RX_PREDICTORFOREACH(_, SEPARATE_BY_SPACE, EXAMPLE_PARAMETER, ## __VA_ARGS__)) {}

#define BUILD_EXAMPLE_METHOD_SELECTOR(return_value, ...) \
    RX_PREDICTORCAT2(RX_PREDICTORCAT2(example_, return_value), RX_PREDICTORFOREACH(_, SEPARATE_BY_SPACE, SELECTOR_PART, ## __VA_ARGS__))
...
#define SWIZZLE_OBSERVE_METHOD_BODY(return_value, ...)                                                                                                       \
       ...
       [self registerOptimizedObserver:^BOOL(RXPredictorObjCRuntime * __nonnull self, Class __nonnull class,                                                     \
...
method_prototype                                                                                                         \
    __unused SEL rxSelector = RX_PREDICTORselector(selector);                                                                     \
...
@interface RXPredictorObjCRuntime (InfrastructureMethods)
@end

// MARK: Infrastructure Methods

@implementation RXPredictorObjCRuntime (InfrastructureMethods)
...
@interface RXPredictorObjCRuntime (swizzle)

@end

@implementation RXPredictorObjCRuntime(swizzle)
...
@implementation RXPredictorObjCRuntime

static RXPredictorObjCRuntime *_instance = nil;
...
+(void)initialize {
    _instance = [[RXPredictorObjCRuntime alloc] init];
    defaultTypeID = CFGetTypeID((CFTypeRef)RXPredictorObjCRuntime.class); // just need a reference of some object not from CF
...
+(void)registerOptimizedObserver:(RXInterceptWithOptimizedObserver)registration encodedAs:(SEL)selector {
    ...
    NSString *methodEncoding = RX_PREDICTORmethod_encoding(exampleEncodingMethod);
...
-(IMP __nullable)ensurePrepared:(id __nonnull)target forObserving:(SEL __nonnull)selector error:(NSErrorParam)error {
    ...
    if (instanceMethod == nil) {
        RX_PREDICTORTHROW_ERROR([NSError errorWithDomain:RXObjCPredictorRuntimeErrorDomain
                                           code:RXObjCPredictorRuntimeErrorSelectorNotImplemented
                                       userInfo:nil], nil);
    }
    ...
    ||  selector == @selector(respondsToSelector:)) {
        RX_PREDICTORTHROW_ERROR([NSError errorWithDomain:RXObjCPredictorRuntimeErrorDomain
                                           code:RXObjCPredictorRuntimeErrorObservingPerformanceSensitiveMessages
                                       userInfo:nil], nil);
    }
    ...
    if (selector == deallocSelector) {
        ...
        NSString *methodEncoding = RX_PREDICTORmethod_encoding(instanceMethod);
        RXInterceptWithOptimizedObserver optimizedIntercept = optimizedObserversByMethodEncoding[methodEncoding];

        if (!RX_PREDICTORmethod_has_supported_return_type(instanceMethod)) {
            RX_PREDICTORTHROW_ERROR([NSError errorWithDomain:RXObjCPredictorRuntimeErrorDomain
                                               code:RXObjCPredictorRuntimeErrorObservingMessagesWithUnsupportedReturnType
                                           userInfo:nil], nil);
        }
        ...
        // default fallback to observing by forwarding messages
            ...
            if ([self forwardingSelector:selector forClass:swizzlingImplementorClass]) {
                return RX_PREDICTORdefault_target_implementation();
            }
    ...
    RX_PREDICTORTHROW_ERROR([NSError errorWithDomain:RXObjCPredictorRuntimeErrorDomain
                                       code:RXObjCPredictorRuntimeErrorUnknown
                                   userInfo:nil], nil);
}
...
-(Class __nullable)prepareTargetClassForObserving:(id __nonnull)target error:(NSErrorParam)error {
    ...
    if (isThisTollFreeFoundationClass) {
        RX_PREDICTORTHROW_ERROR([NSError errorWithDomain:RXObjCPredictorRuntimeErrorDomain
                                           code:RXObjCPredictorRuntimeErrorCantInterceptCoreFoundationTollFreeBridgedObjects
                                       userInfo:nil], nil);
    }
     ...
     * `sentMessage` sets object class to be `_RX_PREDICTORnamespace___KVO__OriginalClass` (subclass of `__KVO__OriginalClass`)
    ...
    if ([target class] != object_getClass(target)) {
        ...
        RX_PREDICTORTHROW_ERROR([NSError errorWithDomain:RXObjCPredictorRuntimeErrorDomain
                                           code:RXObjCPredictorRuntimeErrorObjectMessagesAlreadyBeingIntercepted
                                       userInfo:@{
                                                  RXObjCPredictorRuntimeErrorIsKVOKey : @(isKVO)
                                                  }], nil);
    }
    ...
    if (previousClass != wannaBeClass) {
        ...
        RX_PREDICTORTHROW_ERROR([NSError errorWithDomain:RXObjCPredictorRuntimeErrorDomain
                                           code:RXObjCPredictorRuntimeErrorThreadingCollisionWithOtherInterceptionMechanism
                                       userInfo:nil], nil);
    }
...
-(BOOL)observeByForwardingMessages:(Class __nonnull)swizzlingImplementorClass
...
#endif
    SEL rxSelector = RX_PREDICTORselector(selector);
    ...
    if (!class_addMethod(swizzlingImplementorClass, rxSelector, implementation, methodEncoding)) {
        RX_PREDICTORTHROW_ERROR([NSError errorWithDomain:RXObjCPredictorRuntimeErrorDomain
                                           code:RXObjCPredictorRuntimeErrorSavingOriginalForwardingMethodFailed
                                       userInfo:nil], NO);
    }
    ...
    if (!class_addMethod(swizzlingImplementorClass, selector, _objc_msgForward, methodEncoding)) {
        if (implementation != method_setImplementation(instanceMethod, _objc_msgForward)) {
            ...
            RX_PREDICTORTHROW_ERROR([NSError errorWithDomain:RXObjCPredictorRuntimeErrorDomain
                                               code:RXObjCPredictorRuntimeErrorReplacingMethodWithForwardingImplementation
                                           userInfo:nil], NO);
        }
    }
...
-(Class __nullable)ensureHasDynamicFakeSubclass:(Class __nonnull)class error:(NSErrorParam)error {
    ...
    NSString *dynamicFakeSubclassName = [RX_PREDICTORPREFIX stringByAppendingString:NSStringFromClass(class)];
...
NSInteger RX_PREDICTORnumber_of_dynamic_subclasses(void) {
...
NSInteger RX_PREDICTORnumber_of_forwarding_enabled_classes(void) {
    __block NSInteger count = 0;
    [[RXPredictorObjCRuntime instance] performLocked:^(RXPredictorObjCRuntime * __nonnull self) {
...
NSInteger RX_PREDICTORnumber_of_intercepting_classes(void) {
    __block NSInteger count = 0;
    [[RXPredictorObjCRuntime instance] performLocked:^(RXPredictorObjCRuntime * __nonnull self) {
...
NSInteger RX_PREDICTORnumber_of_forwarded_methods(void) {
    return numberOfForwardedMethods;
}

NSInteger RX_PREDICTORnumber_of_swizzled_methods(void) {
