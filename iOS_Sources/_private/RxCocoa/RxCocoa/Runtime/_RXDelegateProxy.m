...
@interface _RXPredictorDelegateProxy () {
...
@implementation _RXPredictorDelegateProxy
...
+(NSSet*)collectVoidSelectorsForProtocol:(Protocol *)protocol {
    ...
    for (unsigned int i = 0; i < protocolMethodCount; ++i) {
        ...
        if (RX_PREDICTORis_method_with_description_void(method)) {
...
+(void)initialize {
    @synchronized (_RXPredictorDelegateProxy.class) {
...
-(void)forwardInvocation:(NSInvocation *)anInvocation {
    BOOL isVoid = RX_PREDICTORis_method_signature_void(anInvocation.methodSignature);
    NSArray *arguments = nil;
    if (isVoid) {
        arguments = RX_PREDICTORextract_arguments(anInvocation);
